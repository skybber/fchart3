...
Polygon: TypeAlias = list[int]
PolygonDef: TypeAlias = list[object]
PolygonRGB: TypeAlias = tuple[Polygon, RGB]
...
class EnhancedMilkyWay:
    sky_mesh: HTM
    polygon_center_blocks: list[list[int] | None]
    opti_polygon_center_blocks: list[list[int] | None]

    mw_points: Optional[NDArray[np.float32]]          # shape (N,2)
    mw_polygons: list[tuple[list[int], RGB]]          # [(polygon, rgb), ...]
    mw_opti_polygons: Optional[list[tuple[list[int], RGB]]]
    ...
    def __init__(self, milkyway_filename, optim_max_col_diff=None):
        ...
        tm = time()
        self._add_polygons(milkyway_filename)
        tmp = time() - tm
    ...
    def _radec_from_img(self, point: str) -> tuple[float, float]:
        x, y = point.split(',')
        x, y = int(x), int(y)
    ...
    def _add_polygons(self, milkyway_filename: str) -> None:
    ...
    def _create_opti_polygons(self, max_col_diff: float) -> None:
    ...
    def _merge_polygons(self, polygons: list[tuple[list[int], RGB]], max_col_diff: float) -> list[tuple[list[int], RGB]]:
        merges = 0
    ...
    def _merge_edges(self, edges1: list[Edge] | None, edges2: list[Edge] | None) -> list[Edge]:
    ...
    def _edges_to_polygons(self, edges: Sequence[Edge]) -> list[list[int]]:
        if not edges:
            return []
    ...
    def select_polygons(self, field_center: tuple[float, float], radius: float) -> list[int]:
    ...
    def select_opti_polygons(self, field_center: tuple[float, float], radius: float) -> list[int]:
        intersecting_trixels = self.sky_mesh.intersect(RAD2DEG * field_center[0], RAD2DEG * field_center[1], RAD2DEG * radius)
        selection = []
        mask = 1 << (self.sky_mesh.get_depth() * 2 + 3)
